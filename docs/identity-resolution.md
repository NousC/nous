...
- **Strong (surname token in the email):** `099omkarmahale@gmail.com` joining as "Omkar M" matches the existing "Omkar Mahale" because the email local part `omkarmahale` contains the surname `mahale`. High precision, so it is accepted even when the candidate already has an email.
