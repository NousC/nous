...
const BTN_PRIMARY = "inline-flex items-center justify-center gap-1.5 h-9 px-3.5 rounded-lg bg-gray-900 text-white text-[13px] font-semibold hover:bg-gray-800 disabled:opacity-40 transition-colors";
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[560px] max-h-[88vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="text-[16px] font-bold tracking-tight text-gray-900">Create export</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-5 mt-2">
          {/* Range */}
          <div>
            <h3 className="text-[13px] font-semibold text-gray-900 mb-2">Time range</h3>
          ...
          {/* Schema */}
          <div>
            <h3 className="text-[13px] font-semibold text-gray-900 mb-0.5">Schema</h3>
            <p className="text-[12px] text-gray-500 mb-2.5">Pick the fields each exported record should carry.</p>
                ...
                return (
                  <button key={f.key} onClick={() => toggleField(f.key)}
                    ...
                    <span className={`h-4 w-4 rounded flex items-center justify-center flex-shrink-0 border ${
                      on ? "bg-gray-900 border-gray-900" : "border-gray-300"
                    }`}>
                      {on && <Check className="h-3 w-3 text-white" />}
                    </span>
                    <span className="min-w-0">
                      <span className="block text-[13px] font-mono text-gray-800 truncate">{f.label}</span>
                      <span className="block text-[11px] text-gray-400 truncate">{f.sample}</span>
                    </span>
                  </button>
                );
              })}
            </div>
            <p className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-1.5">Record shape</p>
            <pre className="text-[12px] leading-relaxed text-gray-700 font-mono bg-gray-50 border border-gray-100 rounded-lg px-4 py-3 overflow-x-auto whitespace-pre">
{keys.length ? schemaPreview : "// select at least one field"}
          ...
          {/* Format */}
          <div>
            <h3 className="text-[13px] font-semibold text-gray-900 mb-2">Format</h3>
            <div className="inline-flex rounded-lg border border-gray-200 bg-gray-50 p-0.5 gap-0.5">
              {(Object.keys(FMT_LABEL) as Fmt[]).map(f => (
                <button key={f} onClick={() => setFmt(f)}
                  className={`px-3.5 py-1.5 rounded-md text-[12px] font-medium transition-colors ${
                    fmt === f ? "bg-gray-900 text-white" : "text-gray-500 hover:text-gray-900"
                  }`}>
                  {FMT_LABEL[f]}
                </button>
              ))}
      ...
      <div className="px-8 py-7">
        ...
        {rows.length === 0 ? (
          <div className="rounded-xl border border-dashed border-gray-200 py-16 text-center">
            <FileDown className="h-8 w-8 text-gray-300 mx-auto mb-3" strokeWidth={1.5} />
          ...
          <div className="rounded-xl border border-gray-200 overflow-hidden">
            <div className="grid grid-cols-[1.3fr_1fr_1.1fr_0.8fr_0.8fr_1.2fr_84px] gap-4 px-4 py-2.5 bg-gray-50 border-b border-gray-200">
              {["ID", "Status", "Range", "Format", "Records", "Created", "Actions"].map(h => (
                <p key={h} className="text-[11px] font-semibold uppercase tracking-wide text-gray-400">{h}</p>
              ))}
            </div>
            {rows.map(e => (
              <div key={e.id}
                className="grid grid-cols-[1.3fr_1fr_1.1fr_0.8fr_0.8fr_1.2fr_84px] gap-4 items-center px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                <span className="text-[13px] font-mono text-gray-700 truncate">{e.id}</span>
                <span>
                ...
                <span className="text-[13px] text-gray-600">{RANGE_LABEL[e.range]}</span>
                <span className="text-[13px] text-gray-600">{FMT_LABEL[e.format]}</span>
                <span className="text-[13px] text-gray-600 tabular-nums">{e.recordCount.toLocaleString()}</span>
                <span className="text-[12px] text-gray-400">{format(new Date(e.createdAt), "MMM d, HH:mm")}</span>
