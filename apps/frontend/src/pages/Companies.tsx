...
export default function Companies() {
  ...
  const SortHdr = ({ col, label, style, className }: { col:string; label:string; style?:React.CSSProperties; className?:string }) => (
    <button onClick={()=>toggleSort(col)} style={style}
      className={`text-[11px] font-semibold uppercase tracking-wide flex-shrink-0 flex items-center gap-0.5 hover:text-gray-700 transition-colors ${coSort.col===col?"text-gray-700":"text-gray-400"} ${className??""}`}>
        ...
        <div className="flex h-full">
                ...
                {CO_TABS.map(t => (
                  <button key={t.id} onClick={() => setCoTab(t.id)}
                    className={`flex items-center gap-1.5 pb-2.5 text-[13px] font-medium transition-colors flex-shrink-0 ${
                      coTab===t.id ? "text-gray-900 border-b-2 border-gray-900 -mb-px" : "text-gray-400 hover:text-gray-700"
                    }`}>
                    {t.label}
                    {t.count !== undefined && <span className={`text-[11px] ${coTab===t.id?"text-gray-400":"text-gray-300"}`}>{t.count}</span>}
                  </button>
                ))}
              </div>
            </div>
              ...
              {coLoading ? (
                ...
                <div>
                  {detail.contacts.length > 0 && (
                    <>
                      <div className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 mb-3">Contacts ({detail.contacts.length})</div>
                        ...
                        {detail.contacts.map(c => (
                          <div key={c.id} className="flex items-center gap-4 px-4 py-3 border-b border-gray-100 last:border-0">
                            <div className="flex-1 min-w-0">
                              ...
                              {c.title && <div className="text-[12px] text-gray-400">{c.title}</div>}
                            </div>
                            <span className="text-[12px] flex-shrink-0" style={{color:stageColor(c.pipelineStage)}}>{c.pipelineStage}</span>
                            {c.icpScore!=null && <span className="text-[12px] text-gray-400 tabular-nums">{c.icpScore}</span>}
                            {c.lastActivityAt && <span className="text-[12px] text-gray-400 flex-shrink-0">{relTime(c.lastActivityAt)}</span>}
                          </div>
                        ))}
                      </div>
                    </>
                  )}
                  {detail.contacts.length === 0 && <p className="text-[13px] text-gray-400 text-center py-12">No contacts yet</p>}
                </div>
              ) : coTab === "activity" ? (
                coActs.length === 0
                  ? <p className="text-[13px] text-gray-400 text-center py-12">No activity yet</p>
                        ...
                        return (
                          <div key={a.id ?? i} className="py-3">
                            <div className="flex items-center gap-2.5 mb-1">
                              ...
                              <span className="text-[12px] text-gray-500 flex-1 truncate">{a.activity_type?.replace(/_/g," ").toLowerCase()}</span>
                              <span className="text-[12px] text-gray-400 flex-shrink-0">{a.contactName}</span>
                              <span className="text-[12px] text-gray-400 tabular-nums flex-shrink-0">{relTime(a.created_at||a.occurred_at)}</span>
                            </div>
                            {body && <p className="text-[13px] text-gray-700 leading-relaxed pl-[26px]">{body}</p>}
                          </div>
                        );
                      })}
                    </div>
                ...
                coMems.length === 0
                  ? <p className="text-[13px] text-gray-400 text-center py-12">No memories yet</p>
                  : <div className="divide-y divide-gray-100">
                      {coMems.map((m:any) => (
          ...
          <div className="w-64 flex-shrink-0 border-l border-gray-200 px-5 py-5 overflow-y-auto">
            <div className="flex items-center justify-between mb-4">
              ...
              {coSaving && <span className="text-[11px] text-gray-400">saving…</span>}
            </div>
              ...
              {([
                ...
                return (
                  <div key={key}>
                    ...
                    {isReadOnly ? (
                      <div className={`text-[13px] leading-snug break-words ${val?"text-gray-700":"text-gray-300 italic"}`}>{val??"—"}</div>
                    ) : isEditing ? (
    ...
    <div className="h-full overflow-y-auto bg-white">
      <div className="px-8 py-7">
        ...
        <div className="flex items-center justify-between gap-3 mb-4">
          ...
          <span className="text-[12px] text-gray-400 flex-shrink-0 tabular-nums">{filtered.length} of {companies.length}</span>
        </div>
        ...
        <div className="rounded-xl border border-gray-200 overflow-hidden">
          ...
          <div className="flex items-center px-4 py-2.5 bg-gray-50 border-b border-gray-200">
            ...
            <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 flex-shrink-0" style={{width:110}}>Domain</span>
            <span className="text-[11px] font-semibold uppercase tracking-wide text-gray-400 flex-1 min-w-0">Top Contacts</span>
          ...
          {/* Rows */}
          {loading && companies.length === 0 && <div className="text-[13px] text-gray-400 text-center py-12">Loading…</div>}
          {pageRows.map(co => {
            const topContacts = co.contacts.slice(0,3).map(c=>c.name.split(" ")[0]).join(", ");
            return (
            <div key={co.id} className="flex items-center px-4 py-3 border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors group">
              <button onClick={() => setDetail(co)} className="flex items-center gap-2 flex-shrink-0 text-left" style={{width:170}}>
                ...
                <span className="text-[13px] font-medium text-gray-900 truncate">{co.name}</span>
              </button>
              <span className="text-[13px] text-gray-500 flex-shrink-0 truncate pr-2" style={{width:110}}>{co.domain??"—"}</span>
              <button onClick={()=>setDetail(co)} className="text-[13px] text-gray-500 flex-1 min-w-0 truncate pr-2 text-left">{topContacts||"—"}</button>
              <span className="text-[13px] text-gray-500 flex-shrink-0 pr-2" style={{width:92}}>{relTime(co.lastActivityAt)}</span>
              <span className="text-[13px] text-gray-500 flex-shrink-0 truncate pr-2" style={{width:92}}>{co.industry??"—"}</span>
              <span className="text-[13px] text-gray-500 flex-shrink-0 text-right tabular-nums" style={{width:70}}>{co.employeeCount!=null?co.employeeCount.toLocaleString():"—"}</span>
              <span className="text-[13px] text-gray-700 flex-shrink-0 text-right tabular-nums" style={{width:62}}>{co.contactCount}</span>
              ...
              <button onClick={e=>deleteCompany(co.id,e)}
                className="opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 flex justify-end text-gray-300 hover:text-red-500" style={{width:28}}>
                <Trash2 className="h-3.5 w-3.5"/>
              </button>
            </div>
            );
          })}
          {!loading && pageRows.length===0 && <div className="text-[13px] text-gray-400 text-center py-12">No results</div>}
        </div>
        ...
        {totalPages > 1 && (
          <div className="mt-4 flex items-center justify-between">
            <button onClick={()=>setPage(p=>Math.max(0,p-1))} disabled={page===0}
              className="inline-flex items-center gap-1 text-[13px] font-medium text-gray-500 hover:text-gray-900 transition-colors disabled:opacity-30">
