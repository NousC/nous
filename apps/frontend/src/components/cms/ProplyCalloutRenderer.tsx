...
export interface ProplyCalloutOptions {
  HTMLAttributes: Record<string, any>;
}

const ProplyCalloutComponent = ({ node }: any) => {
  ...
  return (
    <NodeViewWrapper className="callout-renderer my-4 not-prose">
      <div className="bg-white border border-[#eeecea] rounded-xl p-5 space-y-3">
        {title && (
          <h3 className="!font-space-mono !font-bold !text-lg !text-[#1a2b3c] !mb-4 !mt-0">
            {title}
          </h3>
        )}
            ...
            {items
              ...
              .map((item: string, index: number) => (
                <li key={index} className="!flex !items-start !gap-3 !mb-0 !pl-0">
                  <span className="!text-[#8a95a3] !mt-1 !flex-shrink-0 !text-lg">•</span>
                  <span className="!font-inter !text-[#3a4a5c] !leading-relaxed">{item}</span>
                </li>
              ))}
          </ul>
        )}
...
export const ProplyCalloutRenderer = Node.create<ProplyCalloutOptions>({
  name: 'callout',
  ...
  addNodeView() { return ReactNodeViewRenderer(ProplyCalloutComponent); },
});
