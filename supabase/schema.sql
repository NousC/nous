...
-- ============================================================
-- 17. VIEW-WRITE LAYER + MAINTENANCE TRIGGERS
--
-- INSTEAD OF handlers that make the v1 views writable (writes ->
-- entities/claims), plus updated_at touches, playground thread bump,
-- subscription plan-name sync, and auto-join of new team members to
-- their team's workspaces. (signup_stats counter is operator-only and
...
-- ── leads view writes (from phase5_leads_to_collections) ──
-- Make the `leads` view writable: inserts/updates fan out to the v2 substrate,
-- deletes remove only the collection membership (the entity + engagement history
-- are never hard-deleted). The delete handler powers "delete selected" in Lists.
...
-- ============================================================
-- people_coverage — attribute coverage estimate with enrichment freshness.
-- "How many <agency founders> do we already have, and how fresh?" — buckets
-- matching person entities by enrichment freshness (never / stale / fresh+verified).
-- See supabase/migrations/2026_06_10_people_coverage.sql.
-- ============================================================
